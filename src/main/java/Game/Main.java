...
class Sprite {
    ...
    /**
     * Renders the sprite by positioning it at the proper location and filling the
     * shape with the specified color.
     *
    ...
    // Checks if the specified point is contained within the visible shape of the
    // sprite
...
/**
 * ConnectionSprite is a subclass of Sprite that represents connections between
 * dots or horizontal connections between sprites.
 * The static method createConnection is used to conveniently create a
 * ConnectionSprite at the specified coordinates and build its shape.
 */
class ConnectionSprite extends Sprite {
    ...
    /**
     * Creates a new ConnectionSprite of the specified type at the given
     * coordinates.
     *
...
/**
 * BoxSprite is a subclass of Sprite that represents the actual boxes made up by
 * the Dot Sprites and ConnectionSprites.
 * <p>
 * The BoxSprite contains references to the four ConnectionSprites which make up
 * its borders.
 * The `isBoxed` method returns `true` when all four of the border
 * ConnectionSprites have `true` `connectionMade` fields.
...
class BoxSprite extends Sprite {
     ...
     * Returns true if all four border ConnectionSprites have true connectionMade
    ...
    /**
     * Creates a new BoxSprite with the specified coordinates and border
     * ConnectionSprites.
     *
     ...
     */
    public static BoxSprite createBox(int x, int y, ConnectionSprite[] horizontalConnections,
...
class Dots extends JFrame implements MouseMotionListener, MouseListener {
     ...
     * This way uses only 1 for loop and keeps
     * track of the current row and column number in colsX, rowsX, colsY, rowsY.
     * colsX and rowsX track the columns and rows
     * for the horizontalConnections while colsY and rowsY track the columns and
     * rows for the vertical connections.
     * The reason to have different fields for vertical and horizontal connections
     * is so that both grids will be filled
     * in left to right and then top to bottom (rows first then columns). This makes
     * it easier to match the connection up
     * to box or boxes it borders. Simple setting colsY=rowsX and rowsY=colsX will
     * put the vertical connections on the
     ...
     * loadBoxes cycles through the box grid in a similar way to loadConnections.
     * There is one less box per side than dots
     * per side.
     */
     ...
     * loadBoxes methods.
     * It uses two nested for loops to iterate over the rows and columns of the dot
     * grid. The order in which
     * the dots are loaded into the dots array does not matter, as they are for
     * visual purposes only.
     * <p>
     * The body of the loop contains the code to build the shape of each dot,
     * setting its position, size,
     * and shape points.
     */
    ...
    /**
     * Starts a new game by initializing the active player, loading connections, and
     * loading boxes.
     * <p>
     * The method sets the active player to PLAYER_ONE. It then calls the
     * loadConnections() method to initialize
     ...
     * <p>
     * The method iterates over the list of horizontal connections and checks if
     * each connection contains the given point (x, y).
     * If a match is found, the matching horizontal connection is returned.
     * <p>
     * If no match is found in the horizontal connections, the method then iterates
     * over the list of vertical connections and
     * checks if each connection contains the given point (x, y). If a match is
     * found, the matching vertical connection is returned.
     * <p>
     ...
     * <p>
     * The method creates a new LinkedList<Boolean> called 'status' with the same
     * length as the 'boxes' list. It then iterates over
     * the 'status' list and assigns the value of 'isBoxed()' method for each box in
     * the 'boxes' list to the corresponding
     * element in the 'status' list.
     * <p>
     * The resulting 'status' list contains the status of each box, where 'true'
     * indicates that the box is boxed and 'false'
     ...
     * <p>
     * The method initializes an integer array called 'scores' with two elements,
     * both set to 0. It then iterates over the
     * 'boxes' collection and checks each 'BoxSprite' object. If a 'BoxSprite'
     * object is boxed and has a non-zero player value,
     * the corresponding element in the 'scores' array is incremented.
     * <p>
     * The resulting 'scores' array contains the scores for each player, where the
     * first element corresponds to Player 1 and
     ...
     * <p>
     * The method takes a 'ConnectionSprite' object as a parameter and performs the
     * following steps:
     * <p>
     * 1. Initializes a boolean variable 'newBox' to false.
     * 2. Retrieves the status of the boxes before the connection using the
     * 'getBoxStatus' method and stores it in a LinkedList<Boolean> called
     * 'boxStatusBeforeConnection'.
     * 3. Sets the 'connectionMade' flag of the 'connection' object to true.
     * 4. Retrieves the updated status of the boxes after the connection using the
     * 'getBoxStatus' method and stores it in a LinkedList<Boolean> called
     * 'boxStatusAfterConnection'.
     * 5. Iterates over each 'BoxSprite' object in the 'boxes' list. For each box,
     * it checks if the status of the box after the connection is different from the
     * status before the connection. If it is different, it sets the 'newBox'
     * variable to true and assigns the 'activePlayer' to the 'player' variable of
     * the box.
     * 6. Checks if 'newBox' is false. If it is false, it switches the
     ...
     * <p>
     * 1. Calculates the scores of the two players using the 'calculateScores'
     * method and stores them in an integer array called 'scores'.
     * 2. Checks if the sum of the scores of both players is equal to the square of
     * 'DOT_NUMBER - 1'.
     * 3. If the above condition is true, it displays a message dialog using
     * 'JOptionPane.showMessageDialog' with the scores of both players and a "Game
     * Over" message.
     ...
     * The method performs the following steps:
     * <p>
     * 1. Retrieves a 'ConnectionSprite' object based on the 'clickX' and 'clickY'
     * coordinates.
     * 2. Checks if the retrieved 'connection' object is null. If it is, the method
     * returns.
     * 3. Checks if the 'connectionMade' property of the 'connection' object is
     * false.
     * - If it is false, the method calls the 'makeConnection' method passing in the
     * 'connection' object.
     ...
     * The method performs the following steps:
     * <p>
     * 1. Calls the 'mouseMoved' method and passes the 'event' object as a
     * parameter.
     *
     * @param event the event
     */
     ...
     * 2. Fills a rectangle with black color using the 'fillRect' method of the
     * 'Graphics' object.
     * The rectangle covers the entire area of the component being painted, which is
     * determined by the 'dim' width and height.
     */
     ...
     * The method performs the following steps:
     * <p>
     * 1. Iterates over the collection of 'horizontalConnections' using a for-each
     * loop.
     * a. Checks if the 'connectionMade' flag of the 'horizontalConnection' is
     * false.
     * - If true:
     * - Checks if the 'horizontalConnection' contains the mouse coordinates
     * (mouseX, mouseY).
     * - If true, sets the 'color' of the 'horizontalConnection' to a specific shade
     * of green (new Color(10, 160, 8, 230)).
     * - If false, sets the 'color' of the 'horizontalConnection' to black.
     * - If false, sets the 'color' of the 'horizontalConnection' to a specific
     * shade of gray (new Color(225, 225, 228, 230)).
     * b. Calls the 'render' method on the 'horizontalConnection' to draw it on the
     * graphical display.
     * <p>
     * 2. Iterates over the collection of 'verticalConnections' using a for-each
     * loop.
     * a. Checks if the 'connectionMade' flag of the 'verticalConnection' is false.
     * - If true:
     * - Checks if the 'verticalConnection' contains the mouse coordinates (mouseX,
     * mouseY).
     * - If true, sets the 'color' of the 'verticalConnection' to a specific shade
     * of green (new Color(10, 160, 8, 230)).
     * - If false, sets the 'color' of the 'verticalConnection' to black.
     * - If false, sets the 'color' of the 'verticalConnection' to a specific shade
     * of gray (new Color(225, 225, 228, 229)).
     * b. Calls the 'render' method on the 'verticalConnection' to draw it on the
     * graphical display.
     */
     ...
     * - If the player is 'PLAYER_ONE', sets the 'color' of the 'box' to
     * 'PLAYER_ONE_COLOR'.
     * - If the player is 'PLAYER_TWO', sets the 'color' of the 'box' to
     * 'PLAYER_TWO_COLOR'.
     * - If false, sets the 'color' of the 'box' to black.
     * 3. Calls the 'render' method on each 'box' to draw it on the graphical
     * display, passing in the 'Graphics' object.
     *
     * @param g the g
     */
     ...
     * The method performs the following steps:
     * <p>
     * 1. Calculates the scores using the 'calculateScores' method and stores them
     * in an integer array 'scores'.
     * 2. Creates three status strings:
     * - 'status': Represents the current player's turn, using the 'activePlayer'
     * variable.
     * - 'status2': Represents the score of Player 1, using the first element of the
     * 'scores' array.
     * - 'status3': Represents the score of Player 2, using the second element of
     * the 'scores' array.
     * 3. Sets the color of the graphics object to white.
     * 4. Draws the 'status' string on the graphical display at the coordinates (10,
     * dim.height-50).
     * 5. Sets the color of the graphics object to the color associated with
     * 'PLAYER_ONE_COLOR'.
     * 6. Draws the 'status2' string on the graphical display at the coordinates
     * (10, dim.height-35).
     * 7. Sets the color of the graphics object to the color associated with
     * 'PLAYER_TWO_COLOR'.
     * 8. Draws the 'status3' string on the graphical display at the coordinates
     * (10, dim.height-20).
     *
     * @param g the g
     */
     ...
     * 3. Calls the following methods to paint different elements on the buffer
     * graphics object:
     ...
     * 4. Draws the buffer image on the graphical display at the coordinates (0, 0)
     * using the 'drawImage' method of the 'Graphics' object passed as a parameter.
     *
     * @param g the g
     */
